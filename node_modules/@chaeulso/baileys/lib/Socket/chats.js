...
export const makeChatsSocket = (config) => {
    ...
    /**
     * Resolve the linked phone-number JID (PN) and LID for a user id.
     * Accepts either representation; the missing side is `undefined` when it
     * cannot be resolved from the local LID mapping store.
     * @returns {Promise<{ lid: string | undefined, phoneNumber: string | undefined }>}
     */
    const findUserId = async (pnLid) => {
        ...
        if (isPnUser(normalizedJid) || isHostedPnUser(normalizedJid)) {
            ...
            const mappedLid = await signalRepository.lidMapping.getLIDForPN(normalizedJid);
            userId.lid = mappedLid ? jidNormalizedUser(mappedLid) : undefined;
        }
        else if (isLidUser(normalizedJid) || isHostedLidUser(normalizedJid)) {
            userId.lid = normalizedJid;
            const mappedPn = await signalRepository.lidMapping.getPNForLID(normalizedJid);
            userId.phoneNumber = mappedPn ? jidNormalizedUser(mappedPn) : undefined;
        }
        else {
