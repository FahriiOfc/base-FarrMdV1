...
`@chaeulso/baileys` lets you build full‑featured **WhatsApp bots, integrations and automations** in pure JavaScript/TypeScript, without a browser. It is the runtime of the well‑known `Baileys` project, packaged for **reliability, performance and compatibility**:
...
- ✅ **Clean, reproducible build** with a green test suite (91 tests) covering the codec, crypto, events, interactive builders and every message type.
...
# pnpm
pnpm add @chaeulso/baileys

# Yarn
yarn add @chaeulso/baileys

# Bun
bun add @chaeulso/baileys

# Deno (npm specifier, Node compat)
#   in your code:  import makeWASocket from "npm:@chaeulso/baileys"
```
...
{
  "dependencies": {
    "@chaeulso/baileys": "github:chaeulso/baileys"          // latest master
    // "@chaeulso/baileys": "github:chaeulso/baileys#v7.1.0" // pinned release tag
    // "@chaeulso/baileys": "github:chaeulso/baileys#semver:^7.1.0" // semver range against tags
  }
}
```
...
yarn add @chaeulso/baileys@github:chaeulso/baileys
...
```ts
import makeWASocket from '@chaeulso/baileys'   // ESM / TS
const { default: makeWASocket } = require('@chaeulso/baileys') // CJS
```
...
```js
const { default: makeWASocket, useMultiFileAuthState, DisconnectReason } = require('@chaeulso/baileys')

// ... identical body to the ESM example
```

That's a complete, auto‑reconnecting echo bot.
...
if (!sock.authState.creds.registered) {
  const phoneNumber = '15551234567' // full international format, no '+'
...
```ts
import { makeInMemoryStore } from '@chaeulso/baileys'
...
```ts
import { getContentType, normalizeMessageContent, extractMessageContent, getMessageTypeInfo } from '@chaeulso/baileys'
...
`@chaeulso/baileys` is hardened for long‑running bots:

- **Graceful reconnects** — handle `connection.update` + `DisconnectReason`; the example above is production‑safe.
...
```ts
import { withTimeout, withRetry, settle } from '@chaeulso/baileys'
...
+ import makeWASocket from '@chaeulso/baileys'
```

```diff
- const { default: makeWASocket } = require('@whiskeysockets/baileys')
+ const { default: makeWASocket } = require('@chaeulso/baileys')
```
...
| Area                  | What changed                                                                     |
| --------------------- | -------------------------------------------------------------------------------- |
| **Package name**      | `@whiskeysockets/baileys` → `@chaeulso/baileys`                                   |
...
npm test             # node --test test/ — 66 offline tests
...
Based on **[Baileys](https://github.com/WhiskeySockets/Baileys)** by Rajeh Taher (WhiskeySockets) and the original project at `github.com/adiwajshing/Baileys`, MIT © 2025. Runtime derived from `7.0.0-rc13`; this fork preserves its public API and adds packaging, typing, hardening, tests and docs.
