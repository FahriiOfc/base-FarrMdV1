...
function isAnimatedWebP(buffer) {
    ...
    try {
        ...
        const hasAnim = buffer.toString('ascii', 12, 16) === 'ANIM';
        return hasAnim;
    } catch (e) {
        return false;
    }
...
function extractAnimDuration(buffer) {
    try {
        const animIndex = buffer.indexOf('ANIM');
...
// ============================================================
// TO VIDEO - FIXED UNTUK ANIMATED STICKER
// ============================================================
...
export async function toVideo(buffer) {
    ...
    try {
        ...
        console.log('[MEDIA] Input buffer size:', buffer?.length || 0);
        ...
        // ============================================================
        // 2. DETEKSI ANIMATED & DURASI
        // ============================================================

        const isAnimated = isAnimatedWebP(buffer);
        console.log('[MEDIA] Is animated WebP:', isAnimated);

        let duration = 3; // default untuk static sticker
        let durationSource = 'default';

        // Coba ekstrak durasi dari ANIM chunk
        if (isAnimated) {
            const animDuration = extractAnimDuration(buffer);
            if (animDuration && animDuration > 0) {
                duration = Math.min(animDuration, 10); // maks 10 detik
                durationSource = 'ANIM chunk';
                console.log('[MEDIA] Duration from ANIM chunk:', duration, 'seconds');
            }
        }

        // Jika tidak dapat dari ANIM, coba probe
            ...
            try {
                ...
                if (probe?.format?.duration && probe.format.duration > 0) {
                    ...
                    if (probeDuration > 0.5) {
                        ...
                        console.log('[MEDIA] Duration from ffprobe:', duration, 'seconds');
                    }
                }
            } catch (probeError) {
        ...
        console.log('[MEDIA] Final duration:', duration, 'seconds (source:', durationSource, ')');
        ...
        // ============================================================
        // 4. KONVERSI KE MP4 - PERBAIKAN UNTUK ANIMATED
        // ============================================================
        ...
        await new Promise((resolve, reject) => {
            ...
            // Untuk animated, kita perlu loop 0 dan durasi sesuai
            if (isAnimated) {
                cmd.inputOptions(['-loop', '0']);
                cmd.outputOptions([
                    '-c:v', 'libx264',
                    '-pix_fmt', 'yuv420p',
                    '-movflags', '+faststart',
                ...
                // Static sticker: loop 1, durasi 3 detik
            ...
            cmd
                .output(outputFile)
                .on('start', (cmdLine) => {
                    console.log('[MEDIA] FFmpeg command:', cmdLine);
                })
                .on('end', () => {
                    console.log('[MEDIA] Video conversion success');
                    resolve();
                })
                .on('error', (err) => {
