// lib/handler.js
// Message Handler - Full Code with Scraper Support + AI Integration + Disable Check + Mode Prefix
...
import { sendWithReply } from './replyHelper.js';

// ============================================================
// 🔥 AI IMPORT
// ============================================================

import aiHandler from '../ai-modules/lib/aiHandler.js';

// ============================================================
// MODE PREFIX - HELPER
// ============================================================

async function isPrefixEnabled() {
    try {
...
export class Handler {
    ...
    async handleMessage(message) {
        try {
            ...
            // ============================================================
            // 📨 INJECT replyWithForward KE CONTEXT
            // ============================================================
            ...
            // Override ctx.reply agar otomatis pakai forwarded
            ...
            // ============================================================
            // 🔥 AI HANDLER - CEK SEBELUM COMMAND (FIXED)
            // ============================================================

            try {
                // PASTIKAN ctx VALID
                if (ctx && ctx.jid && ctx.sender) {
                    const shouldProcessAI = await aiHandler.shouldProcess(ctx);
                    if (shouldProcessAI) {
                        await aiHandler.process(ctx);
                        return; // STOP, jangan lanjut ke command
                    }
                } else {
                    console.log('[HANDLER] ⚠️ ctx tidak lengkap, skip AI handler');
                }
            } catch (error) {
                console.error('[HANDLER] AI Handler error:', error.message);
                // LANJUT KE COMMAND CHECKING
            }
            ...
            if (!isListResponse && !isButtonResponse) {
                ...
                // CEK MODE PREFIX DARI DATABASE
                const prefixEnabled = await isPrefixEnabled();

                let parsed = null;

                // 1. Selalu coba parse dengan prefix dulu
                parsed = serializer.parseCommand(text, config.prefix);

                // 2. Jika prefix OFF dan parse dengan prefix gagal, coba tanpa prefix
                if (!parsed && !prefixEnabled) {
                    ...
                    if (cmd) {
                        ...
                        console.log(`[HANDLER] 🔧 No-prefix command: ${firstWord}`);
                    }
                }
            ...
            // ============================================================
            // FIND COMMAND - WITH DISABLE CHECK
            // ============================================================
            ...
            // ✅ CEK APAKAH COMMAND DISABLED
            if (commandManager.isDisabled(cmd.name)) {
                await ctx.reply(`⚠️ Fitur *${cmd.name}* sedang dalam perbaikan. Silahkan coba lagi nanti.`);
                await ctx.react('⚠️');
                return;
            }
