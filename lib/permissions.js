...
import ownerManager from './ownerManager.js'; // ✅ TAMBAHKAN
...
export class PermissionManager {
    ...
    // ============================================================
    // OWNER CHECK - FIXED (PAKAI DATABASE)
    // ============================================================

    async isOwner(jid, sock) {
        // ✅ CEK DARI DATABASE owners.json
        const cleaned = String(jid).replace(/\D/g, '');
        const fromDB = ownerManager.isOwner(cleaned);
        if (fromDB) return true;
        
        // FALLBACK: cek dari identity (config.ownerNumber)
    ...
    async checkCommandPermissions(ctx, command) {
        ...
        // ✅ CEK OWNER DARI DATABASE + IDENTITY
        const isRealOwner = await this.isOwner(sender, ctx.sock) || fromMe;

        // Owner only commands
        if (command.ownerOnly && !isRealOwner && !fromMe) {
        ...
        if (command.adminOnly && !isRealOwner) {
            if (!isGroup) {
        ...
        // Bot admin required
        if (command.botAdmin && !isRealOwner) {
            if (!isGroup) {
                return { allowed: false, reason: '❌ Command ini hanya dapat digunakan di grup.' };
            }
            if (!isBotAdmin) {
    ...
    #getSender(message) {
        ...
        return identity.normalizeJid(sender);
    }

    #getBotJid(sock) {
        if (!sock?.user) return '';
        return identity.normalizeJid(sock.user.id) || sock.user.id || '';
    }
    ...
    resolveTarget(message) {
        ...
        if (target) {
            target = identity.normalizeJid(target);
            if (!target.includes('@')) {
